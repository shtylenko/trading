...
description: Paper-trade ONE scanner-selected ticker/date from the 09:30 open. The scanner does not reveal its historical breakout time, price, RVOL, or reason. Observe sequential 1-minute data and completed 5-minute candles, then make one objective momentum-entry decision in real time.
---
...
python3 -m trading.llm_trader.replay --seed <N> --from-open --neutral-meta \
    --five-minute-context --format jsonl | head -1
```

`head -1` is deliberate — it shows the `meta` line and nothing else, so you do not
see any bar before the live run. In 4.0 the meta line gives only watchlist context:
...
python3 -m trading.llm_trader.step start --session "$SDIR" --seed <N> \
    --from-open --neutral-meta --five-minute-context
...
latest `resolve` response, plus `high_water`, `bars_since_entry`, and a running bar
index `i` (starts at 0). The feed provides completed 5-minute candles; do **not**
aggregate them yourself. Persist one intent per turn with `recorder log`.
...
gauge), and `bar5_complete`. `bar5_complete` is `null` except after `:04`, `:09`,
`:14`, …; when present it contains completed `o,h,l,c,v` plus `prior_3_high`,
`prior_3_low`, `prior_3_avg_volume`, and `volume_ratio`. `ema20` is the slower pullback
...
expect a fight). There is no anchor price or scanner trigger in 4.0.
...
### 4.0 entry card — this supersedes every older 1-minute entry, arming, and re-entry rule below

**This is the only entry rule in 4.0.0.** The retained 3.x entry text below is
...
If any condition fails, log `OBSERVE`. Do not chase a candle that already completed;
wait for a fresh completed 5-minute break. For a valid entry, set:
...
### Historical 3.x one-minute entry notes — do not apply in 4.0.0
...
2. Soft bailout (failed break / lost VWAP / topping-tail rejection / MACD+price / time stop)? → `EXIT_CLOSE`.
...
6. None → `OBSERVE`. **Do not use `ADD_CLOSE` in 4.0.0.**
...
### Historical re-entry notes — do not apply in 4.0.0
