"""Warrior policy v12: causal scanner-close decision, next-open execution."""
...
POLICY_ID = "warrior_pattern_score_v12"
...
def decisions_for_ticks(
    ...
    return [_identity(value) for value in v1.decisions_for_ticks(
        ticks, execution_config, settings=SETTINGS
    )]
...
def apply_to_session(session_dir: str | Path) -> list[dict[str, Any]]:
    ...
    for field in (
        ...
        "scanner_event_next_open_execution",
    ):
