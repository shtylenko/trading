"""Orchestrate breakout-first-pullback scan → EntryStore."""
...
log = logging.getLogger("llm_trader.breakout_first_pullback")
...
def scan_scope(
    ...
    *,
    ...
    if not symbols:
        raise ValueError("breakout_first_pullback scan has no symbols")

    stats = ScanStats()
    log.info(
        "breakout_first_pullback scan %s → %s | %d symbols | price≥$%.0f",
        cfg.start, cfg.end, len(symbols), cfg.price_min,
    )
    need_spy = cfg.require_spy_above_sma50 or cfg.require_spy_above_sma200
    if market_regime_features is None and need_spy:
        market_regime_features = fetch_market_regime_features(cfg)
    market_ok_dates = None
    if need_spy and market_regime_features is not None:
        market_ok_dates = set()
        for d, rec in market_regime_features.items():
            if cfg.require_spy_above_sma50 and not rec.get("above_sma50"):
                continue
            if cfg.require_spy_above_sma200 and not rec.get("above_sma200"):
                continue
            market_ok_dates.add(d)
    scanned: list[str] = []
    detected: list[Entry] = []
    failures: list[str] = []
    for i, sym in enumerate(symbols, 1):
        ...
        try:
            ...
            raise RuntimeError(f"breakout_first_pullback scan failed closed: {exc}") from exc
        except Exception:
            failures.append(sym)
            log.exception("detect failed for %s", sym)
            continue
        scanned.append(sym)
    ...
    if rate > cfg.max_scan_failure_rate:
        raise RuntimeError(
            f"breakout_first_pullback scan failed closed: {len(failures)}/{len(symbols)} "
            f"({rate:.2%}) failed; database unchanged"
        )
...
def run_scan(
    ...
    *,
    ...
    log.info(
        "done. entries=%d stale_removed=%d table_total=%d wrote %s",
        stats.entries_found, stats.stale_entries_removed, total, txt,
    )
    return stats
