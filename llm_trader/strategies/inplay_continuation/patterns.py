"""In-play gap continuation: impulse → micro-pb → green break (Opp C).

Causal daily RVOL (prior-day). Sim uses WeBull friction via config fee/slip fields.
"""
...
from trading.llm_trader.costs.webull import apply_buy_fill, apply_sell_fill
...
def screen_ticker(ticker: str, cfg: InplayContinuationConfig) -> list[DayCandidate]:
    ...
    # Causal RVOL (E0): prior-day volume only
...
def detect_from_frame(
    ...
    """Reuse micro geometry with in-play impulse threshold."""
    ...
    for i in range(len(df)):
        ...
        features: dict[str, Any] = {
            "signal_kind": "inplay_micro_pb_break",
        ...
        return Entry(
            ...
            day=cand.day,
            time_et=t_et,
            pattern="inplay_continuation",
            entry_px=entry_px,
            bar_close=entry_px,
            reason=(
                f"In-play micro-pb break {t_et}: gap {cand.gap_pct:.1f}% impulse "
                f"${impulse_high:.2f}, {pb_bars}-bar pb → green @ ${entry_px:.2f}."
            ),
...
def detect_entry(cand: DayCandidate, cfg: InplayContinuationConfig) -> Optional[Entry]:
...
def simulate_trade(entry: Entry, cfg: InplayContinuationConfig) -> Optional[SimTrade]:
    ...
    entry_px = apply_buy_fill(raw_open, model)
