...
bar's close; `ARM_BUY_STOP`, `SCALE_LIMIT`, and an entry-attached `bracket` from
the next bar; `SET_STOP` protects from the next bar onward. This ordering prevents
the current bar's already-known low from retroactively stopping a close entry or
retroactively filling a newly placed target.
...
| entry `bracket` | later bars reach engine-derived +R targets | better of open or target, minus sell slippage |
...
### Entry bracket — stage the standard scale plan immediately

Every new `ENTER_CLOSE` or `ARM_BUY_STOP` **must include** the standard `bracket`
below. You name the R multiples and fractions; after the entry actually fills, the
engine derives the dollar targets from its authoritative fill and the structural stop.
It then stages the targets for the **following** bar. That means a target never fills
retroactively on the entry/trigger bar, but it also means a later `SET_STOP` decision
cannot delay the scale plan.
...
The two fractions are calculated from the original filled position, so they bank about
one third at +1R and another third at +2R; the remaining third is the runner. The
engine keeps the protective stop active for every unsold share and still resolves a
bar that reaches a target and the stop as **stop first**. Do not calculate dollar
targets, shares, or fill prices yourself. `SCALE_LIMIT` remains available only for a
separate, exceptional resistance scale—not for the ordinary +1R/+2R plan.
...
4. The entry bracket's +1R/+2R target is now active? → **do not submit a duplicate
   `SCALE_LIMIT`**; the engine will fill it if the next revealed bar reaches it.
...
4. **SCALE OUT in thirds** into strength (§4/§10 — his standard exit plan, all
   three tiers):
   - **First 1/3 at +1R** and **second 1/3 at +2R** are already staged by the
     required entry bracket, using the engine's actual fill and initial stop. They
     become eligible only from the bar after the entry/trigger bar. Move the stop on
     the remainder to **break-even** if not already there.
   - A first clear resistance below +1R or a separate later resistance can justify
     an exceptional `SCALE_LIMIT`, but never submit a second order duplicating an
     active bracket target.
...
# close-confirmed entry — stop + standard bracket required; engine derives fill, shares, and targets
python3 -m trading.llm_trader.recorder log --session "$SDIR" --record \
  '{"i":<i>,"time":"<HH:MM>","thought":"<checklist passed>","action":"ENTER_CLOSE","stop":<price>,"bracket":{"scales":[{"r_multiple":1.0,"fraction":0.333},{"r_multiple":2.0,"fraction":0.333}]},"note":"trigger + standard bracket"}'

# arm a next-bar buy-stop, update protection, scale, or exit
python3 -m trading.llm_trader.recorder log --session "$SDIR" --record \
  '{"i":<i>,"time":"<HH:MM>","thought":"<base is holding>","action":"ARM_BUY_STOP","trigger":<price>,"stop":<price>,"bracket":{"scales":[{"r_multiple":1.0,"fraction":0.333},{"r_multiple":2.0,"fraction":0.333}]},"note":"micro-base + standard bracket"}'
python3 -m trading.llm_trader.recorder log --session "$SDIR" --record \
...
also requires `stop` and `risk_fraction` in `(0,1]`. Every `ENTER_CLOSE` and
`ARM_BUY_STOP` requires the standard two-scale `bracket` shown above.
