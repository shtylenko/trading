...
bar's close; `ARM_BUY_STOP` / `SCALE_LIMIT` from the next bar; engine-managed
pyramid adds at the next bar's open; `SET_STOP` protects from the next bar onward.
This ordering prevents the current bar's already-known low from retroactively
stopping a close entry.
...
| entry `pyramid` add | a prior completed bar qualifies | next bar's open, plus buy slippage |
...
### Entry pyramid — engine-managed starter and adds

Every `ENTER_CLOSE` and `ARM_BUY_STOP` **must include** this plan:

```json
"pyramid":{"starter_fraction":0.333,"max_adds":2}
```

The engine uses about one third of the normally allowed entry shares for the starter,
then manages at most two equal-share adds. It queues add #1 after the first completed
green bar that closes above your actual average entry. It queues add #2 after a later
green `new_high` bar with supportive MACD and either an elevated or early-null
`rvol_bar`. A queued add fills at the **next** bar's open only if that price remains
above the current average entry; otherwise it is cancelled rather than averaging down.

After each actual add, the engine raises the stop as needed so the **entire** position's
open risk remains at or below $40. It never loosens the stop. A stop, scale, or
discretionary exit cancels the plan. Do **not** send `ADD_CLOSE` while this plan is
active—`recorder resolve` shows any queued add and the updated authoritative stop.
...
6. Pyramid plan active? → the engine owns starter/add sizing and timing; never send
   `ADD_CLOSE`. Continue managing stops, scales, and exits from the resolved state.
7. None → `OBSERVE`.
...
6. **ADD / SCALE IN (pyramiding, §10)**. The required entry plan above implements the
   three-step pyramid: starter, proof-of-hold add, then fresh-continuation add. Its
   conditions are mechanical so the one-intent-per-bar contract cannot starve adds
   behind stop updates or scales. It never averages down, never adds after a
   non-confirming bar, and raises the stop to keep total open risk within budget.
...
# close-confirmed entry — required pyramid; engine derives fills, shares, and adds
python3 -m trading.llm_trader.recorder log --session "$SDIR" --record \
  '{"i":<i>,"time":"<HH:MM>","thought":"<checklist passed>","action":"ENTER_CLOSE","stop":<price>,"pyramid":{"starter_fraction":0.333,"max_adds":2},"note":"trigger + engine pyramid"}'

# arm a next-bar buy-stop, update protection, scale, or exit
python3 -m trading.llm_trader.recorder log --session "$SDIR" --record \
  '{"i":<i>,"time":"<HH:MM>","thought":"<base is holding>","action":"ARM_BUY_STOP","trigger":<price>,"stop":<price>,"pyramid":{"starter_fraction":0.333,"max_adds":2},"note":"micro-base + engine pyramid"}'
python3 -m trading.llm_trader.recorder log --session "$SDIR" --record \
...
also requires `stop` and `risk_fraction` in `(0,1]`, but is forbidden while the
required entry pyramid is active.
