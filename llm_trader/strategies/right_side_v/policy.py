"""Deterministic decision policy for causal right-side-v plans.
...
POLICY_ID = "right_side_v_auto_arm_v1"
...
def apply_to_session(session_dir: str | Path) -> list[dict[str, Any]]:
    """Materialize the policy in an unfinalized, fully sealed right_side_v session."""
    ...
    if session.get("strategy") != "right_side_v":
        raise PolicyError("auto-arm policy is available only to right_side_v sessions")
