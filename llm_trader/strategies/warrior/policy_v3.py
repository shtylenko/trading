"""Warrior policy v3: complete-five-minute feed plus a 10:00 entry cutoff.

This is a deliberately narrow selection experiment.  The v5.1 development
panel showed the 10:xx entry cohort losing materially while 09:xx entries were
positive.  All scoring, stops, targets, and exits stay inherited from v1.
"""
...
POLICY_ID = "warrior_pattern_score_v3"
DECISION_SOURCE = "deterministic_policy"
USES_SESSION_EXECUTION_CONFIG = True
SETTINGS = v1.PolicySettings(entry_cutoff="10:00")
POLICY_SPEC = {
    **v1.POLICY_SPEC,
...
def _v3_identity(record: dict[str, Any]) -> dict[str, Any]:
    """Replace common-engine v1 provenance with the immutable v3 identity."""
    out = dict(record)
...
def decisions_for_ticks(
    ...
    """Run the common state machine with v3's explicit time selection rule."""
    return [
        _v3_identity(record)
        for record in v1.decisions_for_ticks(ticks, execution_config, settings=SETTINGS)
    ]


def apply_to_session(session_dir: str | Path) -> list[dict[str, Any]]:
    """Materialize v3 only for the complete-five-minute deterministic contract."""
    ...
    for flag in (
        ...
        "completed_five_minute_entry_required", "entry_bracket_required",
    ):
