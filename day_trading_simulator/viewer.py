...
``_step.json``) are never served, to preserve no-look-ahead.
    ...
    python3 -m trading.day_trading_simulator.viewer                       # open the SPA (session list)
    python3 -m trading.day_trading_simulator.viewer --session <id>        # deep-link a session
    python3 -m trading.day_trading_simulator.viewer --list                # CLI list, no server
    python3 -m trading.day_trading_simulator.viewer --port 8765 --no-browser
"""
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="python -m trading.day_trading_simulator.viewer",
