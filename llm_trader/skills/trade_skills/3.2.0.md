...
| Pillar | Pass |
|---|---|
| Price | `entry_px` in **$3–$20** (candidate small-account band) |
...
- **Grade C (hard gate — structural, not judgment)** — `entry_px` outside
  **$3–$20**, or `rvol < 2`. **STAND DOWN — no trade, regardless of how the bars
  look.** Above $20 the rule-set's stop math breaks: a $23 stock makes a
  $0.10–$0.30 stop pure noise (the corpus uses $0.50–$1.00 stops there, which the
  $40 risk budget can't afford). Below $3, this candidate stands down because the
  one-cent tick, per-share costs, and adverse slippage consume too much of a small
  move under the deterministic execution model. `rvol < 2` remains the corpus's
  hard momentum floor. Log one `STAND_DOWN` record citing the failed pillar; you
  may stop revealing bars and go to Step 4.
