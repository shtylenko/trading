"""Deterministic decision policy for causal cup-and-handle plans.

The scanner already establishes every executable entry parameter on the completed
handle bar.  This policy deliberately adds no discretionary interpretation: it
arms that sealed plan on its revealed setup tick and otherwise observes.  The
generic execution engine remains the sole authority for fills, gap/expiry
cancellation, stops, targets, sizing, and forced exits.

``apply_to_session`` is intentionally routed through :mod:`recorder` rather
than appending JSON directly.  The same revealed-tick, daily-indicator, causal
plan, and engine-target validation applies to deterministic and historical LLM
sessions alike.
"""
...
POLICY_ID = "cup_handle_auto_arm_v1"
...
def decisions_for_ticks(ticks: Iterable[dict[str, Any]]) -> list[dict[str, Any]]:
    """Return one deterministic intent for every already-sequenced stream tick.

    Exactly one causal setup tick is required.  No decision is derived from a
    later tick: an arm contains only scanner-plan values published on that same
    completed setup bar.
    """
    rows = list(ticks)
...
def apply_to_session(session_dir: str | Path) -> list[dict[str, Any]]:
    """Materialize the policy in an unfinalized, fully sealed cup-handle session.

    The function refuses a partly decided session.  It completes all stream and
    plan validation before recording the provenance stamp or the first intent.
    """
    # Local import avoids a package-level recorder → strategy import cycle.
    ...
    if session.get("strategy") != "cup_handle":
        raise PolicyError("auto-arm policy is available only to cup_handle sessions")
    ...
    # Stamp provenance before the first decision.  ``recorder.log`` performs its
    # own lock/read/write around every record; avoid nesting that advisory lock.
