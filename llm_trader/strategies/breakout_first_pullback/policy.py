"""Deterministic decision policy for causal breakout-first-pullback plans.
...
POLICY_ID = "breakout_first_pullback_auto_arm_v1"
...
def apply_to_session(session_dir: str | Path) -> list[dict[str, Any]]:
    """Materialize the policy in an unfinalized, fully sealed breakout_first_pullback session."""
    ...
    if session.get("strategy") != "breakout_first_pullback":
        raise PolicyError("auto-arm policy is available only to breakout_first_pullback sessions")
