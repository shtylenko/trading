"""Scan + multi-year short-hold backtest for BB squeeze long."""
...
log = logging.getLogger("llm_trader.bb_squeeze_long")
...
def run_scan(
    ...
    *,
    ...
    strategy_id: str = "bb_squeeze_long",
) -> ScanStats:
    if symbols is None:
    ...
    log.info("bb_squeeze_long scan %s→%s | %d symbols", cfg.start, cfg.end, len(symbols))
    try:
    ...
    *,
    ...
    for i, r in enumerate(rows):
        ...
        entry = Entry(
            ...
            pattern=r.get("pattern") or "bb_squeeze_long",
        ...
        if tr is not None:
            trades.append(tr)
        if (i + 1) % 50 == 0:
            log.info("  sim %d/%d trades=%d", i + 1, len(rows), len(trades))
...
def run_cost_stress(cfg: BbSqueezeLongConfig, rows: list[dict]) -> dict:
...
def main_backtest(argv: Optional[list[str]] = None) -> int:
    ...
    p = argparse.ArgumentParser(description="BB squeeze long multi-year short-hold backtest")
    p.add_argument("--start", default="2022-01-01")
    p.add_argument("--end", default="2025-12-31")
    p.add_argument("--symbols", nargs="+", default=None)
    p.add_argument("--max-symbols", type=int, default=None)
    p.add_argument("--out", default=None)
    p.add_argument("--skip-scan", action="store_true", help="reuse entries.db only")
    ...
    if args.skip_scan:
        # load + sim only
        store = EntryStore(cfg.db_path)
        rows = [dict(r) for r in store.all_rows(strategy="bb_squeeze_long")]
        store.close()
        trades = []
        for r in rows:
            feats = r.get("features_json")
            ...
            entry = Entry(
                ...
                entry_px=float(r.get("entry_px") or 0),
                bar_close=float(r.get("bar_close") or 0),
                reason="",
                strategy="bb_squeeze_long",
                features=feats or {},
            )
        ...
        years = {y: _agg(ts) for y, ts in sorted(by_year.items())}
        pooled = _agg(trades)
        pos = sum(1 for a in years.values() if a["eff_r"] > 0)
        result = {
            "pooled": pooled,
            "years": years,
            "entries": len(rows),
        ...
        result = run_backtest(
            cfg, symbols=args.symbols, max_symbols=args.max_symbols
        )
    ...
    lines = [
        "# BB squeeze long — multi-year short-hold",
        "",
