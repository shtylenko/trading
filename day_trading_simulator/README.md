...
python3 -m trading.day_trading_simulator.runner

# pipeline smoke test on a slice or explicit names
python3 -m trading.day_trading_simulator.runner --max-symbols 100
python3 -m trading.day_trading_simulator.runner --symbols AAOI ACVA ADEA --float-max 0

# custom window / profile
python3 -m trading.day_trading_simulator.runner --start 2025-01-01 --end 2025-12-31 --profile main
```
...
python3 -m trading.day_trading_simulator.replay --seed 7
python3 -m trading.day_trading_simulator.replay --seed 7 --format jsonl --delay 60 --out-file ticks.jsonl

# the no-look-ahead reader (one streamed tick at a time; --wait blocks for the next)
python3 -m trading.day_trading_simulator.feed --ticks ticks.jsonl --cursor 0 --wait
```
...
```bash
python3 -m trading.day_trading_simulator.step start --session "$SDIR" --seed 7
python3 -m trading.day_trading_simulator.step next  --session "$SDIR"   # the "ping"
```
...
SDIR=$(python3 -m trading.day_trading_simulator.recorder init --ticker EVTV --date 2026-01-13 --seed 7)
...
```bash
python3 -m pytest trading/day_trading_simulator/tests -q
```
