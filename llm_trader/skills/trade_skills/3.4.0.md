...
latest `resolve` response, plus `high_water` (best price since entry), `bars_since_entry`,
`re_entries_used` (0; caps at your **re-entry budget** per §C — default **1**, but a
batch run may raise it and set a **cutoff time** via your run instructions),
`armed_trigger` (a buy-stop level you set
...
- (b) you are `flat` and **out of re-entries** — you have used your entire §C re-entry
  **budget** (default **1**; a batch run may raise it), **or** you have decided not to
  take another, **or** the run's **cutoff time** has passed (after the cutoff you take no
  new/re-entry, so a flat position is terminal).

While you are flat but **still have re-entries left and the cutoff time (if any) has not
passed**, do **not** stop — **keep revealing bars** (logging `OBSERVE`) and watch for the
next qualifying §C setup. Do not stop the loop merely because you are momentarily flat.
...
| `ARM_BUY_STOP` | a later bar reaches `trigger` | worse of open or trigger, plus buy slippage |
...
6. Optional strong continuation? → `ADD_CLOSE` with a non-looser `stop` and a
   `risk_fraction` in `(0,1]`; the engine rejects averaging down or excess open risk.
7. None → `OBSERVE`.
...
Default is **one round trip plus at most one re-entry**. But Cameron's two favorite
setups are *second-leg* entries after a first move fails — "I can always get back in if
there's another setup." So after you've **exited/bailed and are flat**, you MAY take
another re-entry on the same ticker *if* a clean qualifying setup re-forms before the
stream ends — **up to your re-entry budget**.

> **Re-entry budget & cutoff time (run parameters).** Your **budget** is the maximum
> number of §C re-entries allowed this run: **1 by default**, but a batch run may set a
> higher number (or a **cutoff time** in ET) in your run instructions — obey whatever
> those instructions state. Track `re_entries_used`; once it reaches the budget, take **no
> more** re-entries. If a **cutoff time** is given, take **no new or re-entry at/after
> that time** (a flat position then ends the run). With the default budget of 1 and no
> cutoff, this is exactly the classic single-re-entry rule.

Still **one position at a time**; never re-enter while already long, and never to "make
back" the prior loss (that's the loser cognitive loop — every re-entry must be a genuine
A-setup on its own merits, not revenge). Each re-entry independently obeys the mandatory
cooldown and the setup gate below; a bigger budget never lowers the bar for any one entry.
...
If neither re-forms cleanly, **stay flat** — a disciplined round trip with fewer
re-entries than your budget is the success case, not a failure; the budget is a
*ceiling*, never a quota to fill. Apply the same sizing (Step 3), the free-trade/BE rule,
scaling, and exits to every re-entry, and journal each as a distinct trade.
...
# arm a next-bar buy-stop, update protection, scale, or exit
python3 -m trading.llm_trader.recorder log --session "$SDIR" --record \
  '{"i":<i>,"time":"<HH:MM>","thought":"<base is holding>","action":"ARM_BUY_STOP","trigger":<price>,"stop":<price>,"note":"micro-base"}'
python3 -m trading.llm_trader.recorder log --session "$SDIR" --record \
...
Valid actions are `OBSERVE`, `STAND_DOWN`, `ENTER_CLOSE`, `ARM_BUY_STOP`,
`CANCEL_ENTRY`, `SET_STOP`, `SCALE_LIMIT`, `ADD_CLOSE`, and `EXIT_CLOSE`.
Never send `fill_px` or `shares_delta`; that makes the run invalid. `ADD_CLOSE`
also requires `stop` and `risk_fraction` in `(0,1]`.
...
- **One position at a time** (small-account = one round trip per day plus up to your §C
  re-entry budget — default 1, raisable per run; never concurrent, and none at/after the
  run's cutoff time).
