...
the current (closed) bar. If any one fails, do NOT enter: log `OBSERVE`/`STAND_DOWN`
and reveal the next bar.**
...
close is your `avg_entry`** (not `anchor_px`). Set the initial **stop**:
...
- planned **stop** = the low of that pullback/base (the structure that armed it)
- **skip the arm** if `armed_trigger − stop` is wider than ~$0.30 — cut size or pass.
