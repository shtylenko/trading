...
from trading.day_trading_simulator.floats import FloatCache, _fetch_float
from trading.day_trading_simulator.universe import fetch_symbols, _CACHE_TTL_S


def test_float_cache_passes_and_miss(tmp_path, monkeypatch):
    # Isolate cache file
    cache_file = tmp_path / "float_cache.json"
    monkeypatch.setattr("trading.day_trading_simulator.floats._CACHE", cache_file)

    with patch("trading.day_trading_simulator.floats._fetch_float",
...
def test_universe_fetch_and_cache(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("trading.day_trading_simulator.universe._CACHE", cache)
    monkeypatch.setattr("trading.day_trading_simulator.universe.DATA_DIR", tmp_path)
    ...
    with patch("trading.day_trading_simulator.universe.requests.get") as get:
        resp = MagicMock()
