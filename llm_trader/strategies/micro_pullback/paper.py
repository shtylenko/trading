"""Deterministic paper book for micro_pullback (portfolio ON, NML OFF)."""
...
log = logging.getLogger("llm_trader.micro_pullback.paper")

PAPER_CONTRACT = "micro_pullback_paper_book_v0.1.0"
STRATEGY_ID = "micro_pullback"


def build_paper_book(cfg: MicroPullbackConfig, **kwargs):
    return build_short_hold_paper_book(
        ...
        promotion_notes=(
            ...
            "(see stress). Do not live-size. NML gate remains OFF."
        ),
        **kwargs,
    )
...
def write_paper_book(result: dict, out_dir: Path):
    return write_short_hold_paper_book(
        result, out_dir, title="Micro-pullback paper book"
    )
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    p = argparse.ArgumentParser(description="Build micro_pullback paper book")
    p.add_argument("--start", default="2022-01-01")
    p.add_argument("--end", default="2025-12-31")
    p.add_argument("--db", default=None, help="entries.db path (default strategy data)")
    ...
    if not cfg.db_path.exists():
        raise SystemExit(f"entries db missing: {cfg.db_path} — run multi-year scan first")
    ...
    out_dir = Path(args.out_dir) if args.out_dir else (
        Path(__file__).resolve().parents[2] / "batch" / "micro_pullback" / "paper"
    )
