...
§C re-entries below the day's high; the §B.2 time stop reads this), a rolling **5-minute candle** aggregate
...
>
> **Scope: this formula is for CONFIRMED-CLOSE entries.** An **armed** entry uses the
> pre-planned stop computed at arm time (lowest low of the base bars − $0.01 — see
> "Arming the break"), which **overrides** this formula; do not recompute the stop
> on the fill bar. One entry mode, one stop rule — never mix them.
...
- in both cases **green-vol dominance re-established** — the exact §A volume test:
  `green_vol_sum > red_vol_sum` over the last 5 completed bars.
