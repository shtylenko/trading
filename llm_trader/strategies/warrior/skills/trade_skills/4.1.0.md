...
description: Experimental Warrior candidate that combines the 4.0 from-open completed-five-minute entry with deterministic candlebar events and explicit entry/exit confluence scores. Pattern scores describe geometry quality, not probability.
---
...
You are the **trader**. The scanner selected a ticker and date worth watching, but
it does **not** tell you when its historical breakout occurred, at what price, or
why it selected the name. The session starts at **09:30 ET** and is replayed to you
**one 1-minute bar at a time**. Every fifth minute also carries one **completed,
clock-aligned 5-minute candle**, built only from minutes already revealed. You decide
whether a valid setup actually forms — using **Ross Cameron's / Warrior Trading's momentum rules**
...
This is a deterministic OHLC execution environment. You express **intent**; the
recorder alone determines every fill, share count, commission, slippage, and P&L
from the revealed bar. It uses configured adverse assumptions for gaps and bars
whose high/low could hit both a target and a stop. Long only; one position at a
time (small-account "breakout or bailout": one round trip).
...
> - **You do not compute P&L, position, average cost, R, a fill price, or shares.**
>   The recorder derives all of them from your intent. After every revealed tick,
>   `recorder resolve` prints the authoritative position state and any fills.
...
## Step 0 — watch the scanner-selected ticker; do not inherit its trigger
...
python3 -m trading.llm_trader.replay --seed <N> --from-open --neutral-meta \
    --five-minute-context --candlebar-context --format jsonl | head -1
```

`head -1` is deliberate — it shows the `meta` line and nothing else, so you do not
see any bar before the live run. In 4.1 the meta line gives only watchlist context:
`ticker, date, gap_pct, float_shares, prior-day levels, and premarket levels`.
It deliberately omits the scanner's `entry_time`, `entry_px`, `anchor_px`, final-day
`rvol`, and explanation. The first revealed tick is **09:30**, not a known breakout.

## Step 0.5 — watchlist context, not an entry permission

Gap and float are watchlist context only. They do not authorize a trade. Do not
invent a final-day RVOL or a scanner breakout level; wait for the 5-minute signal in
the live stream. A name may be selected by the scanner and still deserve no trade.
...
`init` stamps this run with this candidate's version (so results can be grouped by
rule-set later — Step 4). The immutable skill registry is maintained separately;
never edit a sealed version in place. (Details for maintainers: `MAINTAINING.md`.)
...
python3 -m trading.llm_trader.step start --session "$SDIR" --seed <N> \
    --from-open --neutral-meta --five-minute-context --candlebar-context
...
Keep a small **decision state**: `position`/`shares`/`avg_entry`/`stop` from the
latest `resolve` response, plus `high_water`, `bars_since_entry`, `break_level`,
`entry_bar_high`, `initial_stop_distance`, whether any later bar's `h` exceeded
`entry_bar_high`, consecutive negative-MACD closes, and a running bar index `i`
(starts at 0). The feed provides completed 5-minute candles and candlebar detections;
do **not** aggregate or re-detect them yourself. Persist one intent per turn with
`recorder log`.
...
### Execution state comes from the recorder (mandatory)

Immediately after every `step next` that prints a tick, resolve orders that were
already active **before** that tick. Pass that exact tick's `i`:

```bash
python3 -m trading.llm_trader.recorder resolve --session "$SDIR" --i <i>
```

The JSON response is authoritative: `fills`, `position_shares`, `avg_entry`,
`stop`, `open_risk`, and pending orders. Do not infer a fill from `h`/`l`, invent
shares, or log an `ENTER`/`EXIT` fill yourself. The response is safe to read: it
replays only decisions from prior bars plus the current revealed bar.

Then make one close-confirmed **intent** for this bar and log it. An intent placed
on bar `i` becomes actionable only at the documented point: `ENTER_CLOSE` at this
bar's close; `ARM_BUY_STOP` / `SCALE_LIMIT` from the next bar; `SET_STOP` protects
from the next bar onward. This ordering prevents the current bar's already-known
low from retroactively stopping a close entry.
...
- (b) you are `flat` after your final trade — meaning you have *either* already used
  the one permitted entry, **and** the clock is at or after 11:00 ET. Before 11:00,
  remain in the loop while flat so a valid completed 5-minute setup can form.
...
rvol_bar (this bar's vol ÷ trailing-20-bar average — your minute-level expansion
gauge), `candlebar_patterns`, and `bar5_complete`. `candlebar_patterns` contains
deterministic observations on the current completed 1-minute bar. Each event has
`pattern`, `direction`, `score`, `resolution`, `time`, and geometry `evidence`.
The event `score` is quality in `[0,1]`, **not** a win probability and never an
entry/exit instruction by itself. `bar5_complete` is `null` except after `:04`, `:09`,
`:14`, …; when present it contains completed `o,h,l,c,v` plus `prior_3_high`,
`prior_3_low`, `prior_3_avg_volume`, `volume_ratio`, and its own
`candlebar_patterns` detected only after that 5-minute bar closes. `ema20` is the slower pullback
...
expect a fight). There is no anchor price or scanner trigger in 4.1.

> **Your state is authoritative.** Count every time-based management rule from your
> own actual fill and track `high_water` yourself. The feed contains no recorded-entry
> fields because there is no scanner-defined trade to inherit.

### Execution model — intents only (read first)

Each 1-min tick gives `o, h, l, c, v`; it does **not** reveal the order in which
the high and low occurred. The recorder therefore owns execution and applies this
conservative policy:

| Intent / active order | When the engine acts | Engine price |
|---|---|---|
| `ENTER_CLOSE` | after the current close | close plus configured buy slippage |
| protective stop | a later bar's low reaches `stop` | worse of open or stop, minus sell slippage |
| `SCALE_LIMIT` | a later bar reaches `target` | better of open or target, minus sell slippage |
| `EXIT_CLOSE` | after the current close | close minus configured sell slippage |

The engine sizes entries from the risk budget, buying power, and a participation
cap on reported bar volume; it charges per-share commission. The cap applies to
discretionary entries, scales, and close exits; an already-triggered protective stop
closes the full position at its adverse gap-aware price so partial liquidity cannot
hide tail risk. If a later bar can hit both a scale target and the stop, it applies
the **stop first**. If an armed entry and its stop are both reachable in one bar, it
assumes entry then stop. These are deliberate adverse assumptions, not agent choices.

Consequently, every older phrase in this skill that says "fill at", "sell N shares",
or asks you to log a direct `ENTER` / `SCALE` / `EXIT` fill is superseded by this v3
contract. You select a rule-grounded intent; the recorder supplies the execution.

### 4.1 pattern-confluence entry card — supersedes every older entry rule below

**This is the only entry rule in 4.1.0.** The retained 3.x entry text below is
historical background; do not use it. There are no entries before a completed
5-minute candle and no `ARM_BUY_STOP`, `ADD_CLOSE`, or re-entry in this candidate.

While flat, log `OBSERVE` on every 1-minute tick unless `bar5_complete` is present.
At a completed 5-minute candle, enter with `ENTER_CLOSE` **only if every condition is
true**:

- `time` is before **11:00 ET** (never open a fresh position at or after 12:00 ET);
- `prior_3_high`, `prior_3_low`, and `volume_ratio` are populated — this requires
  three fully completed earlier 5-minute candles;
- the completed candle is green (`c >= o`), closes above VWAP, and its close is in
  the upper third of its range: `h - c < (h - l) / 3`;
- it is a real five-minute break: both `h > prior_3_high` **and** `c > prior_3_high`;
- `volume_ratio >= 1.5`; and
- `macd_hist >= 0` on the current one-minute tick.

These are hard eligibility gates, not the whole decision. If any fails, log
`OBSERVE` with `entry_score=not_scored`. When they all pass, compute this
**transcript-confluence entry score** (maximum 100; sum exact values before making
the decision):

1. **Pattern structure — 0 to 30.** Look at bullish events in the current tick's
   `candlebar_patterns` and `bar5_complete.candlebar_patterns`. Score each eligible
   event as `weight × event.score`, where `bull_flag_break=30`,
   `micro_pullback_break=27`, and `candle_over_candle=22`. Use only the single
   highest result; related detections on one bar do not stack.
2. **Volume — 0 to 25.** `+15` for `volume_ratio >= 1.5`, another `+5` for
   `volume_ratio >= 2.0`, and `+5` when current `rvol_bar >= 1.5` (null gets zero).
3. **Trend — 0 to 20.** `+8` above VWAP, `+6` when `ema9 >= ema20`, and `+6` for
   `macd_hist >= 0`.
4. **Candle quality — 0 to 15.** `+10` for the required green upper-third 5-minute
   close and `+5` only when neither the current 1-minute nor completed 5-minute
   pattern list contains a bearish event.
5. **Timing — 0 to 10.** `+10` before 10:30 ET; `+5` from 10:30 through 10:59.

This score is an explicit, experimental operationalization of the transcript's
pattern/volume/VWAP/EMA/MACD confluence. It is not a probability and its threshold
is not attributed to Cameron. Enter only when the score is **at least 75** and
there is no current `bearish_topping_tail` or `bearish_breakout_failure`. Otherwise
log `OBSERVE`. Put the total and component values in `thought` so the choice is
auditable. Do not chase a candle that already completed; wait for a fresh completed
5-minute break. For a valid entry, save
`break_level = bar5_complete.prior_3_high` in decision state and set:

> **stop = min(bar5_complete.l, bar5_complete.prior_3_low) − $0.01.**

The engine decides whether that structure permits any shares inside the $40 risk,
buying-power, and participation limits. Never tighten this stop just to obtain a
larger position.

### Historical 3.x one-minute entry notes — do not apply in 4.1.0
...
If all hold → submit **`ENTER_CLOSE`** with the structural `stop`. Do not provide
`fill_px` or `shares_delta`: the recorder sizes from risk/buying-power/volume limits
and applies the configured close slippage. Its next `resolve` response is the source
of truth for `avg_entry`, shares, and open risk.
...
Then journal the entry intent with the rule that fired. The engine—not you—turns the
stop distance into an allowed position size.
...
**Log the arm on the bar you decide it** as `ARM_BUY_STOP` with numeric `trigger`
and `stop`. An arm is only valid if it appears in a **previous** bar's logged record;
never fill yourself retroactively at a trigger you did not arm in advance.

On the **next** bar:
- `h ≥ armed_trigger` → the engine fills the already-armed order under the v3
  gap/slippage rules, even if the bar closes red or ugly. That is the real risk of
  buying the break; the failed-break bailout (§B.2) handles it. Read the next
  `resolve` response for the actual fill, size, and retained stop; do **not** log
  `ENTER` yourself.
...
The 4.1 exit-pressure card below replaces the older qualitative soft-bailout
paragraph. Hard stops, break-even handling, scaling, and the 5-minute runner rule
remain unchanged.

**PER-BAR PROCEDURE — on every bar while long, walk this list top-to-bottom and log
the FIRST matching intent:**
1. If `resolve` reports a hard stop fill, you are already flat; do **not** fabricate
   an exit. Log `OBSERVE` or `STAND_DOWN` with the engine event.
2. Immediate failed break or exit-pressure score at least 50? → `EXIT_CLOSE`.
3. Not yet at break-even and price popped ~+$0.10? → `SET_STOP` with stop `avg_entry`.
4. Reached a scale level (+1R, then +2R / resistance)? → `SCALE_LIMIT` with its
   target and `fraction` (normally about `0.333`); the order activates next bar.
5. Runner exit signal on the completed 5-min lens? → `EXIT_CLOSE`.
6. None → `OBSERVE`. **Do not use `ADD_CLOSE` in 4.1.0.**
...
2. **4.1 EXIT-PRESSURE CARD (close-confirmed, exit all).** First preserve Cameron's
   direct breakout-or-bailout rule: on the entry bar or either of the next two bars,
   if `c < break_level`, log `EXIT_CLOSE` immediately without scoring. Otherwise
   compute and cap this score at 100:
   - `+35` when the current 1-minute bar closes below VWAP;
   - up to `+25`: find the highest-quality current bearish
     `bearish_topping_tail` or `bearish_breakout_failure` across the 1-minute and
     completed-5-minute pattern lists, then add `25 × event.score` once (no stacking);
   - `+20` when `bar5_complete` is red and `volume_ratio >= 1.5`;
   - `+10` when `macd_hist < 0` on two or more consecutive completed 1-minute bars;
   - `+10` when at least five bars have closed since the fill, no post-entry new
     high has printed, and `c < avg_entry + 0.25 × initial_stop_distance`.

   Log `EXIT_CLOSE` when the total is **at least 50**; otherwise continue down the
   management list. Record the total and components in `thought`. The score bundles
   the transcript's topping-tail/high-volume-red/slowing-momentum observations with
   price confirmation; it is an experimental decision threshold, not a probability
   or a claim that Cameron teaches a numeric score. A bearish pattern alone can
   never force an exit.
...
5. **RUNNER EXIT (close-confirmed, on the 5-MINUTE lens) + TRAIL.** The runner's
   ...
   favorable move. The feed supplies this view as `bar5_complete`; do not aggregate
   it yourself. It is non-null only after the candle's final minute has closed.
...
### C. RE-ENTRY — disabled in 4.1.0
Do not re-enter after an exit in this candidate. The session is testing one
five-minute-framed first entry only. The historical re-entry notes below do not apply.

### Historical re-entry notes — do not apply in 4.1.0
...
### Each tick → log one intent record (this is the artifact the viewer shows)

After `resolve` and deciding on the current bar, **persist one intent** with
`recorder log`. `i` and `time` must exactly match the revealed tick; the recorder
rejects unrevealed, duplicate, out-of-order, or mismatched records.

```bash
# ordinary observation
python3 -m trading.llm_trader.recorder log --session "$SDIR" --record \
  '{"i":<i>,"time":"<HH:MM>","thought":"<rule-grounded reasoning>","action":"OBSERVE","note":"hold"}'

# close-confirmed entry — stop required; engine derives fill and shares
python3 -m trading.llm_trader.recorder log --session "$SDIR" --record \
  '{"i":<i>,"time":"<HH:MM>","thought":"<checklist passed>","action":"ENTER_CLOSE","stop":<price>,"note":"trigger"}'

# update protection, scale, or exit
python3 -m trading.llm_trader.recorder log --session "$SDIR" --record \
  '{"i":<i>,"time":"<HH:MM>","thought":"<free trade>","action":"SET_STOP","stop":<price>,"note":"break-even"}'
python3 -m trading.llm_trader.recorder log --session "$SDIR" --record \
  '{"i":<i>,"time":"<HH:MM>","thought":"<first scale>","action":"SCALE_LIMIT","target":<price>,"fraction":0.333,"note":"1R"}'
python3 -m trading.llm_trader.recorder log --session "$SDIR" --record \
  '{"i":<i>,"time":"<HH:MM>","thought":"<soft bailout>","action":"EXIT_CLOSE","note":"lost VWAP"}'
```

Use `OBSERVE`, `STAND_DOWN`, `ENTER_CLOSE`, `SET_STOP`, `SCALE_LIMIT`, and
`EXIT_CLOSE` only. Never send `fill_px` or `shares_delta`; that makes the run invalid.

## Step 3 — position sizing is engine-owned

The small profile supplies a $40 **planned** risk budget and $12,000 buying power.
For every entry/add the recorder computes allowed shares from the protective stop,
configured slippage and commissions, buying power, and a cap on bar-volume
participation. A gap through the stop can still lose more than planned risk; that
tail loss is reported rather than hidden. If the structural stop is too wide or
liquidity too thin, the engine may size to zero—treat that as a disciplined pass.

Do not round a share count or calculate P&L/R yourself. The next `resolve` output
is the account state you must manage from.
...
fork a new unsealed version with `batchsim new-version`; never edit a registered
version in place (see `MAINTAINING.md`).
...
- **One position / one entry attempt per day** in this candidate; no re-entry or add.
- Candlebar events are deterministic completed-bar observations. They never override
  eligibility, risk, or execution rules, and their quality score is not a probability.
- **Every action cites a rule** from the strategy doc — no discretionary drift.
- **Deterministic execution only.** The recorder owns fills and shares, applies
  configured slippage/commissions and a volume cap, resolves stop/target ambiguity
  adversely, and exposes actual state through `recorder resolve`. Never report a
  direct fill or override a protective stop.
