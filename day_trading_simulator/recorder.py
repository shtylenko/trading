...
Lifecycle (used by the skill):
    ...
    # 1. start of run
    python3 -m trading.day_trading_simulator.recorder init \
    ...
    python3 -m trading.day_trading_simulator.recorder log --session <dir> \
        --record '{"i":3,"time":"10:23","thought":"…","action":"SCALE",
                   "fill_px":3.90,"shares_delta":-150,"stop":3.75}'

    # 3. end of run (build all artifacts)
    python3 -m trading.day_trading_simulator.recorder finalize --session <dir>
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="python -m trading.day_trading_simulator.recorder",
